...
#member테이블과 외래키 제약조건이 있으므로 board테이블에 먼저 삽입할경우
#에러가 발생된다.
...
/* 자동증가 컬럼에 임의의 값을 삽일할 수 있으나 사용하지
않는 것이 좋다. */
...
/*
	날짜타입의 컬럼에현재날짜를 입력할때 오라클은 sysdate를
...
기존의 게시판을 멀티게시판으로 변경
	...
	질문과답변 : QNA
	FAQ : FAQ
하나의 테이블로 여러개의 게시판을 제작하는 경우 게시판의 구분을위해
flag(플레그)가 필요하다.
*/
-- 공지사항 게시판에 글쓰기
INSERT INTO board (title, content, id, bname)
	VALUES ('여깅공지사항', '내용없음', 'kosmo', 'notice');
